...
namespace Greg
{
    ...
    public class InitWorldJobSystem : ComponentSystem
    {
        ...
        protected override void OnUpdate()
        {
            while (m_Group.Length != 0)
            {
                ...
                var sourceEntity = m_Group.Entity[0]; //set a parent




                var spawner = m_Group.Spawner[0]; //grab stuff
                var center = m_Group.Positon[0].Value;
                var currDepth = m_Group.Depth[0].Value;

                var count = 10;

                EntityManager.RemoveComponent<SpawnNewSystem>(m_Group.Entity[0]);
                EntityManager.RemoveComponent<SystemDepth>(m_Group.Entity[0]);


                Debug.Log(m_Group.Length);
                Debug.Log(spawner.maxDepth);
                Debug.Log(currDepth);
                Debug.Log("break");
