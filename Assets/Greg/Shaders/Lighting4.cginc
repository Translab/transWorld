#if !defined(LIGHTING4_INCLUDED)
#define LIGHTING4_INCLUDED
...
//static half _Frequencyx = .6;
//static half _Frequencyy = .8;
...
InterpolatorsVertex MyVertexProgram (VertexData v) {
	 ...
	 worldPos = worldPos / 80;
	 //v.vertex.x = v.vertex.x + (sin(worldPos.x * _Frequencyx + _Time.y) * _Amplitude);
	 //v.vertex.y = v.vertex.y + (sin(worldPos.y * _Frequencyy + _Time.y) * _Amplitude);

	 //v.vertex.z = v.vertex.z + (sin(worldPos.z * _Frequencyz + _Time.y) * _Amplitude);
	...
	//v.vertex.y += v.normal * sin(worldPos.y * _Frequencyy + _Time.y) * _Amplitude;
	...
	float d1 = sqrt(_U * _U + _W * _W);
	float d2 = sqrt(_V * _V + _W * _W);
	float d3 = sqrt(_U * _U + _V * _V);

	float x;
	float y;
	float z;
	if (_V < 0) {
		y = -abs(sin(worldPos.y *_Frequencyy * d1 - _Time.y*_Frequencyy) * _WaveAmplitude);
	}
	else {
		y = abs(sin(worldPos.y * _Frequencyy * d1 - _Time.y*_Frequencyy) * _WaveAmplitude);
	}
	...
	if (_U < 0) {
		x = -abs(sin(worldPos.x * _Frequencyx * d2 - _Time.y*_Frequencyx) * _WaveAmplitude);
	}
	else {
		x = abs(sin(worldPos.x *_Frequencyx * d2 - _Time.y*_Frequencyx) * _WaveAmplitude);
	}
	...
	if (_W < 0) {
		z = -abs(sin(worldPos.z *_Frequencyz * d3 - _Time.y*_Frequencyz) * _WaveAmplitude);
	}
	else {
		z = abs(sin(worldPos.z * _Frequencyz * d3 - _Time.y*_Frequencyz) * _WaveAmplitude);
	}
	...
	x = abs(sin(worldPos.x * _FreqX * -_Time.y) * _MovementAmplitude) + x;
	y = abs(sin(worldPos.y * _FreqY * -_Time.y) * _MovementAmplitude) + y;
	z = abs(sin(worldPos.z * _FreqZ * -_Time.y) * _MovementAmplitude) + z;



	//float y = sin(_Frequencyy * d - _Time.y) * _Amplitude;
	//float z = sin(_Frequencyy * d - _Time.y) * _Amplitude;
	...
	//float r = 0.8 + sin(6 * _U + _Time.y) * 0.1;
	//r += sin(4 * _V + _Time.y) * 0.1;
	//float s = r * cos( 0.5 * _V);

	//float x = s* sin(_Frequencyz * _V ) * _Amplitude;
	//float y = r* sin(_Frequencyz * 0.5 * _V) * _Amplitude;
	//float z = s* cos(_Frequencyz * _U) * _Amplitude;
