#if !defined(SCULPTUREVERTEX_INCLUDED)
#define SCULPTUREVERTEX_INCLUDED
...
//static half _Frequencyx = 0.145;
//static half _Frequencyy = 0.1;
//static half _Frequencyz = 0.12;
...
InterpolatorsVertex MyVertexProgram (VertexData v) {
	...
	float x = abs(sin(_Frequencyx * -_Time.y) * _MovementAmplitude);
	float y = abs(sin(_Frequencyy * -_Time.y) * _MovementAmplitude);
	float z = abs(sin(_Frequencyz * -_Time.y) * _MovementAmplitude);


/*
	if (_V < 0) {
		y = -abs(sin(_Frequencyy * d1 - _Time.y) * _Amplitude);
	}
	else {
		y = abs(sin(_Frequencyy * d1 - _Time.y) * _Amplitude);
	}
	...
	if (_U < 0) {
		x = -abs(sin(_Frequencyy * d2 - _Time.y) * _Amplitude);
	}
	else {
		x = abs(sin(_Frequencyy * d2 - _Time.y) * _Amplitude);
	}
	...
	if (_W < 0) {
		z = -abs(sin(_Frequencyy * d3 - _Time.y) * _Amplitude);
	}
	else {
		z = abs(sin(_Frequencyy * d3 - _Time.y) * _Amplitude);
	}
