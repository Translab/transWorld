    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                float angleBetween = i.uv.z;
                int picNum = 3;                
                float normalAngles[3];
                float visbility[3];
                
                float entireVisAngleRange = 60.0;
                float shrink = 1.0;
                float visAngleRange = entireVisAngleRange/(float)picNum * shrink;                               
                
                float modedAngle = angleBetween;
                //if(abs(angleBetween) < entireVisAngleRange/2.0){
                //    modedAngle = angleBetween;
                //}
                //if (i.uv.z < - entireVisAngleRange/2.0) {
                //    int zm = floor((- entireVisAngleRange/2.0 - i.uv.z) / entireVisAngleRange);
                //    modedAngle = i.uv.z + (zm+1) * entireVisAngleRange;
                //}
                //if (i.uv.z > entireVisAngleRange/2.0) {
                //    int zm = floor((i.uv.z - entireVisAngleRange/2.0) / entireVisAngleRange);
                //    modedAngle = i.uv.z - (zm+1) * entireVisAngleRange;
                //}
                for(int ii = 0 ; ii < picNum ; ii++){
                    visbility[ii] = 0;
                }
                ////if(abs(angleBetween) < entireVisAngleRange/2.0){
                float w = (modedAngle + entireVisAngleRange/2.0) / visAngleRange;
                int idx = 0 ;
                float res = 0.0f;
                if(w < 0){
                    idx = floor(w) + 1;
                ...
                //if(idx >= 0 && idx < picNum){
                     visbility[idx] = 1.0 - abs(res - 0.5f);
                     if(res > 0.5f) {
                     if(idx == picNum)
                            visbility[0] = 1.0 - abs(1.5f - res);
                        else
                            visbility[idx+1] = 1.0 - abs(1.5f - res);
                     }else{
                        if(idx == 0)
                            visbility[2] = 1.0 - abs(res + 0.5f);
                        else
                            visbility[idx -1] = 1.0 - abs(res+0.5f);
                     }                      
                //}
                //visbility[fmod(idx, picNum)] = 1.0 - abs(res - 0.5f);                
       
                //if(res > 0.5){
                //    visbility[fmod((idx + 1), picNum)] = 1.0 - abs(1.5f - res);                
                //}
                //else {
                //    visbility[fmod((idx - 1), picNum)] = 1.0 - abs(res + 0.5f);                
                //}
